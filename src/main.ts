...
import { existsSync as exists, PathLike, promises as fs, readFileSync } from 'fs';
...
async function validateSubscription() {
  ...
  const upstream = 'sersoft-gmbh/swift-coverage-action';
        ...
        convertedFiles = await core.group('Converting files', async () => {
            ...
            for (const profDataFile of profDataFiles) {
                ...
                for await (const entry of walk(buildDir, false)) {
                    const typesRegex = /.*\.(app|framework|xctest|so)$/; // .so bundles are generated by Swift 6.4+ on Linux
                    ...
                    if (type === 'so') {
                        // .so is only valid on Linux SPM.
                        if (isXcode || process.platform !== 'linux') continue;
                        // Only treat a `.so` as a test bundle when its `-test-runner`
                        // sibling exists — avoids matching plain dynamic-library products.
                        const runnerPath = entry.path.replace(/\.so$/, '-test-runner');
                        if (!await fileExists(runnerPath)) continue;
                    }
